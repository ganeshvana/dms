		...
		for product in self.with_context(prefetch_fields=False):
			...
			if not product_id:
				res[product_id] = dict.fromkeys(
					['qty_available', 'free_qty', 'incoming_qty', 'outgoing_qty', 'virtual_available', 'second_qty_available'],
					0.0,
				)
				continue
			...
			if dates_in_the_past:
				...
				second_qty_available = second_quants_res.get(product_id, 0.0)[0] - moves_in_res_past.get(product_id, 0.0) + moves_out_res_past.get(product_id, 0.0)
			else:
